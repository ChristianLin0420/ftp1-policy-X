...
TASK_LIST=${TASK_LIST:-${1:-"lift_bottle"}}
...
# Max steps per episode (0 = use task default from task_config).
...
RESUME=${RESUME:-0}
RUN_SUFFIX=${RUN_SUFFIX:-"$(basename "${CHECKPOINT_DIR:-ftp1_checkpoint}")"}

if [[ -z "$SUMMARY_ONLY_RUN_ROOTS" ]]; then
  : "${CHECKPOINT_DIR:?Set CHECKPOINT_DIR to an FTP1 checkpoint step directory}"
  : "${DOMAIN_NAME:?Set DOMAIN_NAME to a folder under checkpoint/normalization}"
  python ../scripts/ftp1_preflight.py --checkpoint "$CHECKPOINT_DIR" --domain-name "$DOMAIN_NAME"
fi
...
if [[ -n "$SUMMARY_ONLY_RUN_ROOTS" ]]; then
  ...
  [[ "$RESUME" == "1" ]] && cmd+=(--resume)
  [[ "$MAX_STEPS" -gt 0 ]] && cmd+=(--max_steps "$MAX_STEPS")
fi
