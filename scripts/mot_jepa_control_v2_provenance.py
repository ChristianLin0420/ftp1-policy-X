#!/usr/bin/env python
"""Snapshot or verify the complete paired closed-loop source/artifact closure."""
...
def _verify_schema3_collection_contract(
    ...
    if global_episodes != expected_global_episodes or shard_count != 4 or shard_rank not in {-1, *range(shard_count)}:
    ...
    collector_relative = pathlib.PurePosixPath("scripts_exp_zarr/mot_jepa/control_v2_collect.sbatch")
    ...
    if len(collector_roots) != 1:
        raise ValueError("collection source provenance does not identify one collector source profile")
