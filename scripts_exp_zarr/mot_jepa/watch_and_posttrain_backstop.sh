...
# ---- yield to the primary watcher -------------------------------------------------------
# Both watchers wake within one poll interval of DONE, so the guard below needs the primary to
# have actually called submit.sh (which mkdir -p's the run directory) before it is checked.
# Ten minutes is far longer than the primary needs to snapshot and submit.
say "DONE seen. Yielding 600s to let the primary watcher claim this pair."
sleep 600

# ---- idempotence guard ------------------------------------------------------------------
# This copy exists as a backstop for a primary watcher that may already have launched this
# pair. Submitting a duplicate would not corrupt anything, but two runs sharing an EXP_NAME
# share a run directory and would interleave checkpoints. If either arm's run directory
# already exists, the primary won; exit without launching.
for ARM in real surr; do
  if [[ -d "${RUN_ROOT}/mot_jepa_stage3/s3_${TAG}_${ARM}" ]]; then
    say "s3_${TAG}_${ARM} already exists -- primary watcher won the race. Not launching."
    exit 0
  fi
done
